...
export default function QASection() {
  ...
  useEffect(() => {
    ...
    const fetchQuestions = async () => {
      try {
        console.log('Fetching predefined questions from QA server');
        const response = await fetch('/api/question-answer');
        if (response.ok) {
          const data = await response.json();
          console.log('Received predefined questions:', data.questions);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('Frontend: Submitting question to API', {
        question,
        fingerprint: currentDocumentMetadata?.fingerprint,
        documentName: currentDocumentMetadata?.name
      });
      
      // Call the internal Next.js API to get the answer - using same endpoint as for predefined questions
      const response = await fetch('/api/question-answer', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      console.log('Frontend: Question API response status:', response.status);
