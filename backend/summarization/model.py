...
class SummarizerModel:
    ...
    def initialize(self, model_path="models/summary_model/model_weight.pth", tokenizer_path="t5-base"):
        ...
        if not self._is_initialized:
            try:
                ...
                absolute_path = "c:/Users/eshwa/OneDrive/Pictures/content/RAW/models/summary_model/model_weight.pth"
                if os.path.exists(absolute_path):
                    print(f"Loading model from absolute path: {absolute_path}")
                    self._model.load_model(absolute_path)
                elif os.path.exists(model_path):
                    print(f"Loading model from provided path: {model_path}")
                    self._model.load_model(model_path)
                else:
                    print(f"Model weights not found, using base t5 model")
    ...
    def summarize(self, text):
        ...
        try:
            # Direct generation without extra checks for more reliable output
